...
#define eps0(x) (ROTR32(x, 7) ^ ROTR32(x, 18) ^ ((x) >> 3))
#define eps1(x) (ROTR32(x, 17) ^ ROTR32(x, 19) ^ ((x) >> 10))
...
void sha256_update(sha256_ctx_t *ctx, const uint8_t *data, uint32_t len) {
    ...
    if (len >= free) {
        for (uint32_t i = 0; i < free; i++) ctx->buffer[idx + i] = data[i];
        sha256_transform(ctx, ctx->buffer);
        for (uint32_t i = free; i + 64 <= len; i += 64) {
            sha256_transform(ctx, data + i);
        }
        idx = 0;
    }
    for (uint32_t i = idx; i < idx + (len & 63); i++) {
        ctx->buffer[i] = data[(len & ~63) + (i - idx)];
    }
}
...
void sha256_final(sha256_ctx_t *ctx, uint8_t hash[32]) {
    ...
    for (int i = 0; i < 8; i++) {
        ctx->buffer[56 + i] = (ctx->count[1] >> (24 - i * 8)) & 0xFF;
        ctx->buffer[60 + i] = (ctx->count[0] >> (24 - i * 8)) & 0xFF;
    }
    sha256_transform(ctx, ctx->buffer);
    for (int i = 0; i < 8; i++) {
